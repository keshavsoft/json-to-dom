...
describe("json-to-dom v39 Engine", () => {
    ...
    test("4. Build spec array (e.g. list or table rows)", () => {
        const uninstall = installMockDocument();
        try {
            const specArray = [
    ...
    test("6. Supports parameter naming convention (inSpec, inDomIdToPushTo)", () => {
        const uninstall = installMockDocument();
        try {
            const inSpec = {
                tagName: "div",
            ...
            const element = buildSpecElement({ inSpec });
            assert.equal(element.tagName, "DIV");
