...
import buildSpecElement, {
    ...
    bindActions,
    meta
} from "../src/v31/index.js";
import listeners from "../src/v31/chapters/chapter3_activation/listeners/index.js";
import data, { guardTags } from "../src/v31/chapters/chapter1_inspection/standards/index.js";
import validateTag from "../src/v31/chapters/chapter1_inspection/validateTag.js";

import extractFormValues from "../src/v31/chapters/chapter3_activation/listeners/v2/extractFormValues.js";
import resetForm from "../src/v31/chapters/chapter3_activation/listeners/v2/resetForm.js";
...
describe("json-to-dom v31 3-Chapter Story Architecture", () => {
    test("1. Meta information & Standards Schema", () => {
        assert.equal(meta.version, "v31.0");
        assert.equal(typeof meta.description, "string");
        assert.ok(data.schema, "data.schema exists");
        assert.equal(data.schema.title, "HTML Tags Standards Dictionary");
        assert.ok(data.tags, "data.tags exists");
        assert.ok(data.tags.div, "data.tags.div exists");

        const guardReport = guardTags({ inTags: data.tags, inGuards: data.guards });
        assert.equal(guardReport.isValid, true);
        assert.equal(guardReport.errors.length, 0);

        // Level 0: Non-string, non-object value at root
        const invalidRoot = guardTags({
            inTags: { invalidKey: 123 },
            inGuards: data.guards
        });
        assert.equal(invalidRoot.isValid, false);
        assert.ok(invalidRoot.errors.some(e => e.includes("Level 0")));

        // Level 1: Missing required key
        const missingReq = guardTags({
            inTags: { mytag: { allowsTextContent: true } },
            inGuards: data.guards
        });
        assert.equal(missingReq.isValid, false);
        assert.ok(missingReq.errors.some(e => e.includes("missing required key 'allowsChildren'")));

        // Level 1: Wrong type (string instead of boolean/array)
        const wrongType = guardTags({
            inTags: { mytag: { allowsTextContent: "yes", allowsChildren: false } },
            inGuards: data.guards
        });
        assert.equal(wrongType.isValid, false);
        assert.ok(wrongType.errors.some(e => e.includes("must be boolean or array")));

        // Level 1: Unrecognized key
        const extraProp = guardTags({
            inTags: { mytag: { allowsTextContent: true, allowsChildren: false, invalidProp: 123 } },
            inGuards: data.guards
        });
        assert.equal(extraProp.isValid, false);
        assert.ok(extraProp.errors.some(e => e.includes("unrecognized key 'invalidProp'")));

        // Level 2: Non-string item inside array
        const nonStringItem = guardTags({
            inTags: { mytag: { allowsTextContent: true, allowsChildren: false, allowedAttributes: [123] } },
            inGuards: data.guards
        });
        assert.equal(nonStringItem.isValid, false);
        assert.ok(nonStringItem.errors.some(e => e.includes("Level 2") && e.includes("must be string")));
    });

    test("1b. Streamlined validateTag validates against tags.json", () => {
        // Valid div with allowed attribute
        const validReport = validateTag({
            spec: { tagName: "div", attributes: { class: "container", role: "main" } }
        });
        assert.equal(validReport.isValid, true);
        assert.equal(validReport.errors.length, 0);

        // Void tag with children -> error
        const voidReport = validateTag({
            spec: { tagName: "input", children: [{ tagName: "span" }] }
        });
        assert.equal(voidReport.isValid, false);
        assert.ok(voidReport.errors[0].includes("Void tag '<input>' cannot have children"));

        // Missing tagName -> error
        const missingTagReport = validateTag({ spec: { textContent: "oops" } });
        assert.equal(missingTagReport.isValid, false);
        assert.ok(missingTagReport.errors[0].includes("Missing or invalid 'tagName'"));
    });
